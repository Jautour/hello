pipeline {
    ...
    environment {
        ...
        REMOTE_USER = "ec2-user" //원격(srping) 서버 사용자
        REMOTE_HOST = "54.253.41.98" //원격(srping) 서버 IP(Public IP)
        REMOTE_DIR = "/home/ec2-user/deploy"  // 원격 서버에 파일 복사할 경로
        SSH_CREDENTIALS_ID = "4421a30a-ab53-4cd2-b47f-6efb86a83056" // Jenkins SSH 자격 증명 ID                              
    }
